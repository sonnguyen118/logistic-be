...
menuModel.toggleEnabledMenu = async (id) => {
  ...
  const queryUpdateArticle =
    "UPDATE articles SET isEnabled = !isEnabled WHERE menu_id = ?";
...
menuModel.updateMenuRole = async (menuId, roleId) => {
  const queryUpdateMenu = "UPDATE menu SET role = ? WHERE id = ?";
  const queryUpdateArticle = "UPDATE articles SET role = ? WHERE menu_id = ?";

  try {
    const [rows, fields] = await pool.query(queryUpdateMenu, [roleId, menuId]);
    await pool.query(queryUpdateArticle, [roleId, menuId]);
    return rows.affectedRows;
  } catch (err) {
    throw err;
  }
...
menuModel.addMenu = async (menu, transaction) => {
  const query =
    "INSERT INTO menu (name, link, description, parent_id) VALUE(?,?,?,?)";
  try {
...
menuModel.updateMenuById = async (menu, transaction) => {
  ...
  const queryUpdateMenu =
    "UPDATE menu SET name =?, link =?,description =?,role = ? WHERE id = ?";
  const queryUpdateArticle = "UPDATE articles SET role = ? WHERE menu_id = ?";
  try {
    const paramsUpdateMenu = [
      menu.name,
      menu.link,
      menu.description,
      menu.role,
    ...
    const updateMenu = await transaction.execute(
      queryUpdateMenu,
      paramsUpdateMenu
    );
    const paramsUpdateArticle = [menu.role, menu.id];
    avoidUndefined(paramsUpdateArticle);
    const updateArticles = await transaction.execute(
      queryUpdateArticle,
      paramsUpdateArticle
    );
...
menuModel.getArticlesByMenuId = async (id) => {
  const query =
    "SELECT m.*, a.id as articleId,a.title,a.link,a.description,a.create_at,a.update_at, a.tag" +
    " FROM (" +
...
menuModel.orderByMenu = async (firsId, secondId, transaction) => {
  ...
  const [result2, fields2] = await transaction.execute(query2, [
    firsId,
    secondId,
  ]);
